...
func TestConfig_Validate(t *testing.T) {
	validConfig := func() *Config {
		return &Config{
			...
			PushgatewayURL:  "http://pushgateway",
			Retry: RetryConfig{
				MaxAttempts:  3,
	...
	t.Run("empty pushgateway URL", func(t *testing.T) {
		cfg := validConfig()
		cfg.PushgatewayURL = ""
		assert.ErrorContains(t, cfg.Validate(), "pushgateway_url is required")
	})
...
func TestLoader_Load_Defaults(t *testing.T) {
	...
	assert.Equal(t, DefaultPushgatewayURL, cfg.PushgatewayURL)
...
`
	...
	assert.Equal(t, "http://custom-pushgateway:9091", cfg.PushgatewayURL)
